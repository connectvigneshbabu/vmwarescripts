...
#  			Output will show in RED if any datasotre's Free Space is less then 100
# History:
#
#
#*****************************************************************************************
...
$Result += Get-View -ViewType Datastore | Where-Object {$_.Name -notmatch "pag"} | Select-Object -Property Name, 
  ...
  @{N="FreeSpace";E={[math]::Round(((100* ($_.Summary.FreeSpace/1GB))/ ($_.Summary.Capacity/1GB)),0)}} | sort -Property "FreeSpaceGB"
    ...
    $HTML += "<HTML><BODY><Table border=1 cellpadding=0 cellspacing=0 id=Header><caption><font size=1 color=black><h1 align=""left""> <font size=4 color=""#00008B""> Please take action on datastores where free space is </font> <font size=4 color=""#FF6262""> less than 100GB </font> </h1></font>
    <h2 align=""right""><font size=1 color=""#00008B"">Date: $D </font></h2></caption>
    ...
    {
        if($Entry.FreeSpaceGB -lt "100")
            {
                  $HTML += "<TR bgColor=#FF6262>"
				  
							  }
            else
            {
                  $HTML += "<TR bgColor=#E0FFC2>"
            }
            $HTML += "
                                    <TD>$($Entry.Name)</TD>
...
#Send mail-> 
    ...
    $msg.Subject = "VMware Production Data Store Usage Report" # Subject of the email.
 
 foreach($vc in $vcs) 
     { 
     $MailTextT =  Get-Content ($SCRIPT_PARENT + "\Report\192*.html") -ErrorAction SilentlyContinue
     $Sig =  "<html><p class=MsoNormal><o:p>&nbsp;</o:p></p><B> Executed from the ID, <p> </B></p></html>"
     $Top = "<html>  <b>  </b></html>"
